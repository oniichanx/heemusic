...
export default class Queue extends Command {
    ...
    public async run(client: heemusic, ctx: Context): Promise<any> {
            ...
            return await ctx.sendMessage({
                embeds: [
                    this.client
                        ...
                        .setDescription(
                            `เขากำลังดูดไข่ผมม: [${player.current.info.title}](${
                                player.current.info.uri
                            }) - ชายผู้สั่งให้ผมเริ่มดูดไข่ โดยนาย: ${player.current?.info.requester} - Duration: ${
        ...
        const queue = player.queue.map(
            (track, index) =>
                `${index + 1}. [${track.info.title}](${track.info.uri}) - ชายผู้สั่งให้ผมเริ่มดูดไข่ โดยนาย: ${
