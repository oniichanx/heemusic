...
import { Dispatcher, heemusic } from '../structures/index.js';
...
function neb(embed: EmbedBuilder, player: Dispatcher, client: heemusic): EmbedBuilder {
    ...
    return embed
        ...
        .setDescription(
            ...
            } • \`[${client.utils.formatTime(player.current.info.length)}]\` - ชายผู้สั่งให้ผมเริ่มดูดไข่ โดยนาย ${
                        ...
                        .send({
                            embeds: [
                                embed
                                    ...
                                    .setDescription('เกิดข้อผิดพลาดขณะค้นหา.'),
                            ],
                        })
                        .then(msg => {
                            setTimeout(() => {
                    ...
                    await message.channel
                        .send({
                            embeds: [
                                embed
                                    .setColor(client.color.red)
                                    .setDescription('ขอโทษที่น้าาาแต่มึงให้กูหาเหิ้ยไรเนี่ย.'),
                            ],
                        })
                        .then(msg => {
                            setTimeout(() => {
                            ...
                            .send({
                                embeds: [
                                    embed
                                        .setColor(client.color.red)
                                        .setDescription(
                                            `คิวโมกกูยาวเกินกูจำกัดสิทธิ์คิวโมก แค่ ${client.config.maxQueueSize} ดิว.`
                                        ),
                                ],
                            })
                            .then(msg => {
                        ...
                        .send({
                            embeds: [
                                embed
                                    .setColor(client.color.main)
                                    .setDescription(
                                        `เพิ่ม [${res.data.info.title}](${res.data.info.uri}) ลงในคิวโมกแล้ว.`
                                    ),
                            ],
                        })
                        .then(msg => {
                            ...
                            .send({
                                embeds: [
                                    embed
                                        .setColor(client.color.red)
                                        .setDescription(
                                            `ดิว ผู้หญิงในlist มึงเยอะไปกูเด้าได้ แค่ ${client.config.maxPlaylistSize} ดิว ควยกูก็ไม่แข๊งงแล้ว.`
                                        ),
                                ],
                            })
                            .then(msg => {
                        ...
                        .send({
                            embeds: [
                                embed
                                    .setColor(client.color.main)
                                    .setDescription(
                                        `เพิ่ม [${res.data.tracks.length}](${res.data.tracks[0].info.uri}) ลงในคิวโมกแล้ว.`
                                    ),
                            ],
                        })
                        .then(msg => {
                        ...
                        .send({
                            embeds: [
                                embed
                                    .setColor(client.color.main)
                                    .setDescription(
                                        `เพิ่ม [${res.data[0].info.title}](${res.data[0].info.uri}) ลงในคิวโมกแล้ว.`
                                    ),
                            ],
                        })
                        .then(msg => {
        ...
        const embed = client
            ...
            .setDescription(
                `[${track.info.title}](${track.info.uri}) - \`[${client.utils.formatTime(
                    track.info.length
                )}]\` - ชายผู้สั่งให้ผมเริ่มดูดไข่ โดยนาย ${track.info.requester}`
            )
            .setImage(icon);
        await m
            .edit({
...
async function updateSetup(client: heemusic, guild: any): Promise<void> {
